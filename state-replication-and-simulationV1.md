...
2. **Get pools and assets of router**
   [`Portal.getRouterPools(router)`](https://docs.nabla.fi/developers/portal#getrouterpools) → returns assets  supported by that router.
   ```solidity
   function getRouterAssets(address router) external view returns (address[] memory assets
  
   ```
   [`Router.poolByAsset(asset)`](https://docs.nabla.fi/developers/router#poolbyasset) → returns pool for corresponding asset
   ```solidity
   function poolByAsset(address asset) external view returns (address pool)
...
const portalAbi = [
  ...
  "function getRouterAssets(address router) external view returns (address[] assets"
];
...
async function discoverPools() {
  ...
  for (const routerAddress of routerAddresses) {
    const assetAddresses: string[] = await portal.getRouterAssets(routerAddress);
    const router = new ethers.Contract(routerAddress, routerAbi, provider );
    const pools: Pool[] = assetAddresses.map((asset, i) => ({
      address: await router.poolByAsset(asset),
      asset,
    }));
